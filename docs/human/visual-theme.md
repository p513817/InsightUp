...
- 底部 Dock 的選取狀態以柔和內層 highlight、陰影或色塊呈現，不建議只靠高對比白色 border。
- 浮動主操作按鈕維持 `56px`，次要浮動操作維持 `48px`，並保留 safe-area bottom offset。
- 相鄰可點擊控制至少保留 `8px` 間距，單一點擊區至少 `44px`。
- 個人目標與競賽個人細節應共用進度元件，避免同一種進度在不同頁面有不同語意。
- 綜合進度與項目進度高度需一致；差異可以用色調、透明度、文字層級或背景處理，不要用不同尺寸造成誤解。
- 負值進度不一定使用鮮紅。若不是錯誤或危險狀態，優先用深藍系搭配方向、缺口、斜紋、透明度或文字來表達退步。
- Dashboard 趨勢線使用線性回歸。開啟趨勢時，原始資料線應變淡，趨勢線可以用較粗、半透明的螢光筆樣式覆蓋。
- 趨勢好壞顏色應參考指標方向規則：體重、脂肪、體脂率、內臟脂肪下降較好；肌肉、分數、基礎代謝、建議熱量上升較好。
- 競賽列表卡片的進度是競賽時間進度，不是個人成員目標進度。
