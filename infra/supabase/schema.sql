...
create table public.inbody_segments (
  id uuid primary key default gen_random_uuid(),
...
create index inbody_records_user_chart_idx
  on public.inbody_records (user_id, is_included_in_charts, recorded_at desc)
  where deleted_at is null;

create index inbody_records_source_type_idx
  on public.inbody_records (source_type, scan_status);

create index inbody_segments_record_id_idx
