...
create or replace function public.list_friend_records(input_friend_user_id uuid)
returns table (
  id uuid,
  user_id uuid,
  recorded_at date,
...
as $$
  ...
  order by r.recorded_at asc, r.created_at asc;
$$;

comment on function public.list_friend_records(uuid) is
'Returns non-deleted InBody records for a friend only when the signed-in user has that friend in user_friendships.';
